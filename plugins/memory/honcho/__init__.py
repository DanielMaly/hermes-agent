...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Base context prewarm is non-blocking and fail-open. The first
        # substantive turn still performs a query-anchored fetch, so this
        # generic warm-up is only a latency optimization.
        if self._recall_mode in {"context", "hybrid"}:
            try:
                self._manager.prefetch_context(self._session_key)
            except Exception as exc:
                logger.debug("Honcho context prewarm failed: %s", exc)

        # Generic dialectic prewarm is incompatible with latest-message
        # rewriting: when rewriting is enabled, wait for the user's message
        # and anchor the first dialectic query to it instead.
        if (
            self._recall_mode in {"context", "hybrid"}
            and cfg.prefetch_generic_context
        ...
        elif self._recall_mode in {"context", "hybrid"}:
            logger.debug(
