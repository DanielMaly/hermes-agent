...
CURATOR_REVIEW_PROMPT = (
    ...
    "When consolidating, preserve distinct scenarios as SEPARATE reference "
    "entries rather than collapsing them into a single over-generalized rule "
    "in the umbrella's SKILL.md. Each reference entry should retain its own "
    "applicability conditions — the specific context, preconditions, and "
    "constraints under which that scenario applies. A SKILL.md with an index "
    "pointing to five well-scoped reference entries is more useful than one "
    "paragraph that averages over those five cases. Consolidate into "
    "STRUCTURE (index + references), not into ABSTRACTION (one vague rule).\n\n"
    "When patching SKILL.md, preserve applicability conditions: include the "
    "scope and the 'why', not just the 'what'.\n\n"
