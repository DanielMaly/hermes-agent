...
def init_agent(
    ...
    if not agent.quiet_mode:
        if compression_enabled:
            # Report the active engine's own threshold token count — for a
            # plugin engine the host compression_threshold is not necessarily
            # in effect (#44439). If the fork's absolute cap is configured,
            # keep that visible too.
            _cc = agent.context_compressor
            _cap_note = ""
            if compression_threshold_tokens is not None and compression_threshold_tokens > 0:
