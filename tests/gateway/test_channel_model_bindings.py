...
Covers the per-channel model binding feature: config bridging, adapter
resolution (exact channel, parent fallback, string shorthand, malformed
warnings), runtime application in ``_resolve_session_agent_runtime``
(provider+model, provider-only default model, session /model override wins),
synthetic-event source lookup, WhatsApp JID keys, and the /model command's
channel-binding default route.
"""
...
from gateway.platforms.base import MessageEvent, resolve_channel_model_binding
...
def _patch_runtime(monkeypatch, *, channel_runtime=None):
    """Patch the gateway runtime resolvers used by _resolve_session_agent_runtime."""
    ...
    if channel_runtime is not None:
        monkeypatch.setattr(gateway_run, "_resolve_channel_binding_runtime_kwargs", channel_runtime)
...
def test_channel_model_binding_applies_provider_and_model(monkeypatch):
    ...
    _patch_runtime(
        monkeypatch,
        channel_runtime=lambda binding: {
...
def test_session_model_override_wins_over_channel_binding(monkeypatch):
    ...
    _patch_runtime(
        monkeypatch,
        channel_runtime=lambda binding: pytest.fail("channel binding should not be resolved when /model override exists"),
    )
...
async def test_retry_preserves_channel_model_binding(monkeypatch):
    ...
    runner._async_session_store = SimpleNamespace(  # type: ignore[assignment]
        ...
        get_or_create_session=AsyncMock(return_value=SimpleNamespace(session_id="session-1", last_prompt_tokens=10)),
...
def test_whatsapp_jid_resolves_channel_model_binding():
    ...
    assert resolve_channel_model_binding(config_extra, "999999999999@s.whatsapp.net", None) is None


def test_model_command_uses_channel_binding_as_default_route(monkeypatch):
    """/model with no args shows the channel binding as the current route when no session override."""
    from gateway.slash_commands_model import _ModelSwitchContext

    ctx = _ModelSwitchContext(
        session_key="agent:main:discord:thread:12345", source=_make_source(),
        config_path=None, persist_global=False,
    )
    ctx.current_model = "global-model"
    ctx.current_provider = "openrouter"
    ctx.current_base_url = "https://global.example/v1"

    ctx.apply_channel_binding({"provider": "anthropic", "model": "claude-sonnet-4-6"})

    assert ctx.current_model == "claude-sonnet-4-6"
    assert ctx.current_provider == "anthropic"
    assert ctx.current_base_url == "https://global.example/v1"  # binding has no base_url → unchanged
